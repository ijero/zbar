...
public class QRCodeReaderView extends SurfaceView
  ...
  public void startSpot(int delay) {
  ...
  private ProcessDataDelegate delegate;
  ...
  public void setDelegate(ProcessDataDelegate delegate) {
  ...
  public void onPreviewFrame(byte[] data, Camera camera) {
    ...
    if(!isStop){
      ...
      decodeFrameTask.setDelegate(delegate);
  ...
  private static class DecodeFrameTask extends AsyncTask<byte[], Void, Result> {
    ...
    @Override protected Result doInBackground(byte[]... params) {
      ...
      if(delegate != null){
          return delegate.processData(params[0], view.mPreviewWidth, view.mPreviewHeight);
      }else {
  ...
  public interface ProcessDataDelegate{
    Result processData(byte[] data, int width, int height);
  }
}
